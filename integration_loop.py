...
r1 = 1
r2 = [10, 25, 50, 75, 100, 250, 500, 750, 1000, 2500, 5000,
      7500, 10000]  # r2 list to demonstrate convergence
...
for i in range(len(r2)):
    # loop over r2, determine integral value at each r2
    riemann_list.append(func.riemann(gamow_integral, r1, r2[i], 10000))
    ...
    # probably a slicker way of doing this, reinitialize list of sample points
    sample_pts = np.linspace(r1, r2[i], 10000)
    gamow_sampled = gamow_integral(sample_pts)  # sample integrand

    # pass sample integrand and sample points to scipy
    scipy_simpson.append(simpson(gamow_sampled, sample_pts))
...
axs[0].scatter(r2, scipy_simpson,
               label="Simpson's, Scipy Implementation", alpha=0.5)

axs[1].scatter(r2, trapz_list, label="Trapezoidal")
axs[1].scatter(r2, analytic_list, label='Analytic')
axs[1].scatter(r2, scipy_trapz,
               label="Trapezoidal, Scipy Implementation", alpha=0.5)
...
############ ERROR FOR STEP SIZE #######################################
r2 = 11
r1 = 1  # chosen more or less at random

step_sizes = np.array([0.00001, 0.0001, 0.001, 0.01, 0.1,
                      0.25, 0.5, 1])  # list of step sizes
...
for i in range(len(N_steps)):
    ...
    simpson_val = func.simpson(gamow_integral, r1, r2, int(
        N_steps[i]))  # compute integral with certain step size
    trapz_val = func.trapezoidal(gamow_integral, r1, r2, int(N_steps[i]))

    # compare computed value to analytic value
    simpson_error.append(np.abs(analytic_val - simpson_val))
