...
for i in range(len(alpha)):
    euler_list.append(func.euler_explicit(0, 10, 0, end_time,
                      10000, alpha[i], omega_0))  # append solution to list
    rk4_list.append(func.runge_kutta4(
        0, 10, 0, end_time, 10000, alpha[i], omega_0))

    # state vector for scipy, needed to be vectorized
    def state_for_scipy(t, u, alpha=alpha[i], omega_0=omega_0):
        out1 = u[1]
        out2 = -2 * alpha * u[1] - (omega_0**2) * u[0]

        return np.array([out1, out2])

    rk4_scipy_list.append(solve_ivp(state_for_scipy, (0, end_time), (0, 10),
                          method="RK45", vectorized=False, rtol=1e-9))  # append scipy solution to list


fig, axs = plt.subplots(figsize=(12, 8), ncols=3,
                        sharey=True, layout='tight')  # plotting
...
for i in range(len(alpha)):
    axs[0].plot(euler_list[i][2], euler_list[i][0],
                label=rf'$\zeta$ = {alpha[i]}')
    axs[1].plot(rk4_list[i][2], rk4_list[i][0], label=rf'$\zeta$ = {alpha[i]}')
    axs[2].plot(rk4_scipy_list[i].t, rk4_scipy_list[i].y[0],
                label=rf'$\zeta$ = {alpha[i]}')
...
R = 0
L = 1
C = 1  # initialize new initial conditions (hehe)
...
# determine solution from each method
rk4_sol = func.runge_kutta4(0, 10, 0, end_time, 10000, alpha, omega_0)
...
plt.plot(euler_sol[2], analytic_sol,
         linestyle='dashed', label="Analytic Solution")
plt.legend(loc='best')
plt.xlabel('Time (s)')
plt.ylabel('I(t) (A)')
...
step_sizes = np.array([0.0001, 0.0005, 0.001, 0.005,
                      0.01, 0.05, 0.1])  # vary step size
...
for i in range(len(step_sizes)):
    rk4_sol = func.runge_kutta4(0, 10, 0, end_time, int(
        N_steps[i]), alpha, omega_0)  # determine solution
    euler_sol = func.euler_explicit(
        0, 10, 0, end_time, int(N_steps[i]), alpha, omega_0)
    analytic_sol = analytic_for_R0(euler_sol[2], 10, omega_0)
