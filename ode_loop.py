...
R = np.array([0.25,0.5,1,1.5,2,2.5])
L = 1
C = 1
...
def analytic_for_R0(t, dot_I_init, omega_0):
    return (dot_I_init)/(omega_0)*np.sin(omega_0*t)

end_time = 3 * np.pi
euler_list, rk4_list, rk4_scipy_list = [] ,[] ,[]
for i in range(len(alpha)):
    euler_list.append(func.euler_explicit(0, 10, 0, end_time, 10000, alpha[i], omega_0))
...
plt.plot(rk4_sol[2], rk4_sol[0], label='Runge-Kutta 4')
...
step_sizes = np.array([0.0001,0.0005,0.001,0.005,0.01,0.05,0.1])
N_steps = end_time/step_sizes
...
for i in range(len(step_sizes)):
    rk4_sol = func.runge_kutta4(0,10,0,end_time,int(N_steps[i]),alpha,omega_0)
